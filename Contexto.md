...
*   **Incrustación Automatizada de OpenJDK 17 y 21**: Se desarrollaron e implementaron los pasos de empaquetado en el workflow de CI para descargar automáticamente los runtimes de Java `jre17-ios-aarch64` y `jre21-ios-aarch64` (OpenJDK nativos portados a iOS) de los builds oficiales de `PojavLauncherTeam/android-openjdk-build-multiarch`. Los archivos se extraen de forma segura y se colocan bajo `Payload/AmetrineLauncher.app/java_runtimes/jre17` y `jre21`, empaquetándose con soporte para enlaces simbólicos (`zip --symlinks`) lo cual es crucial para prevenir la rotura de librerías nativas de Java como `libjli.dylib`.
*   **Integración de MoltenVK**: Se configuró y automatizó la descarga e incrustación de la librería dinámica de **MoltenVK** (`libMoltenVK.dylib` compilada para iOS/ARM64) desde el repositorio oficial de KhronosGroup. Esta librería se coloca en `Payload/AmetrineLauncher.app/Frameworks/libMoltenVK.dylib` y se pseudo-firma con `ldid` junto al binario principal, permitiendo que las llamadas de renderizado Vulkan (esenciales para mods modernos de Minecraft como VulkanMod) se traduzcan en caliente hacia Metal, la API de aceleración gráfica nativa de Apple Silicon.

## 2. Tareas Pendientes (Lo que Falta)

*   **Verificación y Pruebas del Pipeline Completo**: Subir y ejecutar el workflow de GitHub Actions para corroborar el correcto empaquetado del archivo `.ipa` compilado cruzado en la nube con todas sus dependencias embebidas (Qt 6 iOS, JRE 17/21, MoltenVK, GLFW Controller Bypass).
